...
{
    /// <summary>
    /// Registers entries in CIR under their federation GUID.
    ///
    ...
    /// <summary>
    /// Runs a <c>GetRegistry</c> query and returns the registries that matched.
    /// </summary>
    /// <remarks>
    /// The read half of the registry, and the reason this client is shared: a
    /// participant resolving a governed identity has to ask CIR what it already
    /// holds before deciding to write anything.
    ///
    /// An unmatched query returns an empty list rather than raising. "CIR holds
    /// no such entry" is an ordinary answer -- it is precisely the answer that
    /// tells a caller the mapping has not been established yet.
    /// </remarks>
    Task<IReadOnlyList<CirRegistry>> GetRegistryAsync(
...
public sealed class CirRestClient(HttpClient http, ILogger<CirRestClient> logger) : ICirClient
{
    ...
    public async Task<IReadOnlyList<CirRegistry>> GetRegistryAsync(
        IReadOnlyList<CirFilter> filters, CancellationToken ct)
    {
        // POST, not GET: the filter is a nested AND/OR structure carrying regex
        // values, and '+' in a query string decodes to a space.
        using var response = await http.PostAsJsonAsync(
            "queries/registry", new GetRegistryRequest(filters), Json, ct);

        // An empty registry and a 404 are the same answer to the caller: CIR
        // holds nothing matching. Returning empty for both keeps "not yet
        // mapped" off the exception path, where it would be indistinguishable
        // from CIR being broken.
        if (response.StatusCode == HttpStatusCode.NotFound)
            return [];
        ...
        if (!response.IsSuccessStatusCode)
        {
            ...
            throw new CirClientException(
                $"CIR returned {(int)response.StatusCode} querying the registry: {body}");
        }

        var result = await response.Content.ReadFromJsonAsync<GetRegistryResponse>(Json, ct);

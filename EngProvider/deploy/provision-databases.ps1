<#
.SYNOPSIS
    Creates the ENG databases on acme-sql-server.

.DESCRIPTION
    Two databases, deliberately separate from the sandbox databases:

        acme-db-eng-dev    development instance
        acme-db-eng-prod   production instance
...
Write-Host ''
Write-Host 'The app creates its own schema at startup (Eng__AutoCreateSchema=true).' -ForegroundColor Yellow

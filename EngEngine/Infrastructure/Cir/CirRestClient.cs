...
namespace EngEngine.Infrastructure.Cir;

/// <summary>
/// The slice of the CIR model this engine writes.
///
/// Declared locally rather than shared, matching every other engine: CIR is
/// reached over HTTP as a peer system, and a ProjectReference would let its
/// storage concerns compile into this one. The duplication across engines is
/// deliberate and is the same trade RegLocationEngine, MmsEngine and CmsEngine
/// already make.
/// </summary>
...
/// nothing.
///
/// For the entries this engine writes, <paramref name="EntryIdInSource"/> is
/// ENG's own key for the twin and <paramref name="EntrySourceId"/> is ENG. The
/// federation GUID lives in the entry's Cirid -- that separation is the whole
/// point of the registration: it is what lets a consumer holding the GUID find
/// its way back to ENG's row.
/// </remarks>
...
{
    /// <summary>
    /// Registers entries under their federation GUID.
    ///
    /// Returns the number actually written. Entries that were already there are
    /// not an error: republication is expected here, and a bootstrap that failed
    ...
    /// </summary>
    /// <remarks>
    /// Tolerant in the same way as registration: an entry already gone is the
    /// state the caller wanted, so it counts as nothing-removed rather than
    /// raising. That tolerance is required, not merely convenient -- a delete
    /// can fail partway and be repeated, and the second pass necessarily finds
    /// the entries the first one removed.
    /// </remarks>
...
public sealed class CirRestClient(HttpClient http, ILogger<CirRestClient> logger) : ICirClient
{
    ...
    /// already registered, and a duplicate in one category would otherwise
    /// suppress a new entry in another -- CIR writing nothing while the caller
    /// reports nothing written, which is indistinguishable from success. Split,
    /// only the duplicate category conflicts and the new one still lands.
    /// </summary>
        ...
        CirRegistry registry, CirCategory category, bool createCirid, CancellationToken ct)
    {
        ...
        // normal outcome of re-announcing a twin this engine has seen before, so
        // it is reported as nothing-written rather than raised: the identity CIR
    ...
    /// is therefore all-or-nothing in exactly the case that matters: a repeated

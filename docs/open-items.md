...
## The sandbox and the engines point at different CIR and ISBM deployments

Raised 2026-09 while making day zero clear CIR. **This is the reason day zero
cannot currently reach everything it claims to.**

There are two of each piece of shared infrastructure, and which one you get
depends on which config you read:

| Consumer | ISBM | CIR | Registry |
|---|---|---|---|
| Sandbox participants (`personality.yaml`) | `isbm-func-44p2f3n6dv7p4` | `cir-func-44p2f3n6` | `OIIE-SANDBOX` |
| Every engine (app settings) | `acme-api-isbm-dev` | `acme-api-cir-dev` | `acme` |

Nothing is misconfigured in isolation — each half works. The problem is that
anything reasoning about "the" broker or "the" registry silently addresses only
one of them, and the sandbox is the component that most needs to see both.

Day zero is the case that exposed it. It enumerates channels from the
participants' ISBM client, so it cleaned the legacy broker (4 channels rebuilt,
9 removed) while 22 per-iTwin channels on `acme-api-isbm-dev` — the ones every
engine actually publishes to — were left untouched. The CIR half was fixed by
configuring `Sandbox:CirProviderBaseUrl` and dropping the `acme` registry over
REST, which is a workaround: it makes day zero address a second deployment
explicitly rather than removing the reason there is one.

The decision to make is which deployment survives:

- **Retire the legacy pair.** Repoint the personality packs at
  `acme-api-isbm-dev` / `acme-api-cir-dev` and delete
  `isbm-func-44p2f3n6dv7p4` / `cir-func-44p2f3n6`. One broker, one registry,
  and day zero's channel enumeration becomes correct for free. The risk is
  whatever still depends on `OIIE-SANDBOX` — the scenario routes register
  through `CirRegistrationService`, so those paths need checking first.
- **Keep both and teach the sandbox about both.** Honest about the demo having
  two environments, but every teardown, health check and diagnostic then has to
  enumerate a set rather than a single client, and each one is a place to
  forget the second.

The first is much the stronger position. The split appears to be historical
rather than intended: the `acme-*` resources were provisioned for the engine
work and the personality packs were never moved. Until it is resolved, treat
any statement of the form "day zero cleans the channels" as meaning "the
legacy broker's channels".

Note also that the per-iTwin channels are created by `BootstrapChannelsAsync`
during site ingestion, so they are not in the participant registry and would
not be recreated by day zero even once it can see them. That is correct —
they should reappear when sites are re-established — but it means the expected
post-day-zero state is *no* per-iTwin channels, not a rebuilt set.

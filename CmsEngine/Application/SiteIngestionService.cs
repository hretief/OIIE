...
public sealed class SiteIngestionReport
{
    ...
    /// <summary>Sites newly created in CMS.</summary>
...
/// <summary>
/// Receives SyncSites from ENG and records the site in CMS.
///
/// This is how a plant comes to exist in CMS. Nothing else creates one: sites
/// are not seeded, and CmsProvider is reachable only through its own REST API,
/// so the only route from "ENG published a site" to "CMS knows about it" is this
/// service. That is deliberate -- a condition monitoring system that invented
/// its own plants would be asserting something only the enterprise can.
///
    ...
    ILogger<SiteIngestionService> logger)
{
    ...
    /// between.
    ///
    /// This engine's session is its own. REG-LOCATION reads the same channel
    /// through a separate session, so both receive every site rather than
    /// competing for them.
    /// </summary>
    ...
    public async Task<SiteIngestionReport> DrainAsync(CancellationToken ct)
    {
        ...
        if (string.IsNullOrWhiteSpace(_options.CmsBaseUrl))
        {
            report.Note = "CmsBaseUrl is not configured, so sites cannot be recorded.";
        ...
        while (report.MessagesRead < _options.MaxMessagesPerPoll)
        {
            ...
            catch (Exception ex)
            {
                ...
                // is a plant CMS cannot attach a single measurement to.
                logger.LogError(
                    ex,
        ...
        IsbmMessage message, SiteIngestionReport report, CancellationToken ct)
    {
        ...
        var upserts = mapped
            .Select(m => new SiteUpsert(
                ...
                // in CMS that no publisher ever asserted.
                ParentSiteId: null,
                Country: null,
                Region: null,
                Status: null))
        ...
        var upsertResult = await cms.UpsertSitesAsync(upserts, ct);
        ...
        {
            logger.LogWarning(
                "CMS rejected site {Key}: {Reason} (transient: {Transient}).",
                rejection.Key, rejection.Reason, rejection.Transient);
        }

        // A site CMS refused for a reason that may pass is grounds to keep the
    ...
    /// <summary>
    /// Registers the sites in CIR under CMS's own key space.
    ///
    /// This is what makes CMS's site resolvable by systems that do not speak
    /// CMS. The CIRID is the site's federation GUID, the same one REG-LOCATION
    /// registered against its own identifier -- which is precisely how the
    /// registry relates the two without either system learning the other's
    /// keys.
    /// </summary>
        ...
        IReadOnlyList<SiteMappingResult> mapped, CancellationToken ct)
    {
        ...
        var entries = mapped
            .Select(m => new CirEntry(
                // CMS's own identifier for the site. SiteId doubles as it here
                ...
                new CirRegistry(
                    ...
                    [
                        new CirCategory(
                            Id: "ITWIN-SITE",
                            SourceId: _options.SourceId,
                            Description: [new CirLocalizedText("CMS sites")],
                            Entries: entries)
                    ])
            ],
            CreateCirid: false);
        ...
        {
            // Not fatal. The site is already in CMS, and failing here would leave
            // it unrecorded and blocked behind a CIR that is still down.
            logger.LogError(ex, "Registering {Count} CMS site(s) in CIR failed.", entries.Count);
            return 0;
        }
    }
}

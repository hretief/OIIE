...
namespace CmsEngine.Infrastructure.Cms;

/// <summary>
/// The slice of CMS's contract this engine writes.
///
/// Declared locally rather than referenced. CmsProvider emulates a customer
...
public sealed class CmsClientException(string message) : Exception(message);

public interface ICmsClient
{
    /// <summary>
    /// Creates or updates sites in CMS.
    ///
...
public sealed class CmsRestClient(HttpClient http) : ICmsClient
{
        ...
        IReadOnlyList<SiteUpsert> sites, CancellationToken ct)
    {
        ...
        {
            throw new CmsClientException(
                "CMS reported every site in the batch as transiently rejected.");
        }
        ...
        if (!response.IsSuccessStatusCode)
        {
            ...
            throw new CmsClientException(
                $"CMS refused the site upsert ({(int)response.StatusCode}): {Excerpt(body)}");
        }

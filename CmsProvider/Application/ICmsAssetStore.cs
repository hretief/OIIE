namespace CmsProvider.Application;

/// <summary>
/// Configuration for the CMS emulation.
/// </summary>
...
/// CMS's own vocabulary — Site, Asset, AssetNumber — and knows nothing of CCOM,
/// Segments, BODs or functional locations. A customer system does not know it is
/// being integrated with, and translation is the integrator's job, not CMS's.
///
...
public interface ICmsAssetStore
{
    ...
    Task<CmsSite?> FindSiteAsync(Guid siteId, CancellationToken ct);
    ...
    /// a consumer resolving a CMS site cannot be expected to know which.
    /// </summary>
    Task<SiteUpsertResult> UpsertSitesAsync(
        IReadOnlyList<SiteUpsert> sites,
        CancellationToken ct);

    Task<CmsAsset?> FindAssetAsync(Guid assetId, CancellationToken ct);
    ...
    /// </summary>
    Task<CmsAsset?> FindAssetByNumberAsync(Guid siteId, string assetNumber, CancellationToken ct);

    Task<IReadOnlyList<CmsAsset>> GetAssetsAsync(Guid? siteId, CancellationToken ct);

    /// <summary>
    /// The asset types CMS recognises.
    ///
    ...
    /// </summary>
    Task<IReadOnlyList<CmsAssetType>> GetAssetTypesAsync(CancellationToken ct);

    Task<CmsAssetType?> FindAssetTypeByCodeAsync(string assetTypeCode, CancellationToken ct);
...
///
/// SiteID is required and is the caller's own identifier for the site. CMS mints
...
public sealed record CmsAssetType(
    Guid AssetTypeId,
...
/// describing a segment rarely knows the customer's type taxonomy, and CMS falls
...
/// claiming knowledge CMS has no reason to trust.
/// </summary>
...
/// <summary>
/// One persisted asset. AssetTypeId is reported back because CMS may have
...
/// <summary>
/// Something CMS declined to store, and why. Shared by the site and asset paths:

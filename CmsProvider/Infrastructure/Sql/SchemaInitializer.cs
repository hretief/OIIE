    ...
    ILogger<SchemaInitializer> logger) : IHostedService
{
    ...
    public async Task StartAsync(CancellationToken ct)
    {
        ...
        {
            logger.LogWarning("Cms__SqlConnectionString is not configured; skipping schema initialization.");
            return;
        }

        try
        {
            var ddl = await ReadEmbeddedAsync("CmsProvider.Infrastructure.Sql.schema.sql");
            ...
            logger.LogInformation("CMS schema verified.");
        }
        catch (Exception ex)
        {

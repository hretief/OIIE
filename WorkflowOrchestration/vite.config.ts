  ...
  return {
    ...
    server: {
      ...
      proxy: {
        ...
        // by default). Proxying rather than calling it directly keeps the browser on one
        // origin in development, so CORS and cookie behaviour are not something to debug
        // while building screens.
        //
        // secure: false because the API presents the ASP.NET development certificate,
        // which Node does not trust by default -- without it every call fails with
        // UNABLE_TO_VERIFY_LEAF_SIGNATURE and looks like the API is down.
        '/admin': {
          target: process.env.SANDBOX_API || 'https://localhost:7241',
          changeOrigin: true,
          secure: false,
        },
      },

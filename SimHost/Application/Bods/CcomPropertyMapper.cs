...
/// <summary>
/// Maps between the Sandbox classification model (§6.5) and CCOM's native attribute
/// model. The correspondence is close enough to be structural rather than invented:
///
...
/// </summary>
public sealed class CcomPropertyMapper
{
    private readonly IClassificationSource _source;

    public CcomPropertyMapper(IClassificationSource source)
    {
        _source = source;
    }
    ...
    public (List<IncomingProperty> Properties, List<string> ClassKeys) Extract(Entity entity)
    {
        ...
        foreach (var setForEntity in entity.AttributeSetForEntity)
        {
        ...
        properties.AddRange(entity.Attribute.Select(ToIncoming));
